import { Convert } from "../services/services/userService";
import { addPost, deletePost } from "../services/services/postService";
...
import { processBuikPDFUpload, processBulkCompressUpload } from "../services/services/utils";// Used to sanitize input to prevent XSS
...
export async function uploadContentHandler(req, res) {
    ...
    try {
        ...
        if (req.files && Object.keys(req.files).length > 0) {
            const fileArray = Object.values(req.files).flat() as fileUpload.UploadedFile[];
...
export async function uploadMCQHandler(req, res) {
...
export async function uploadFileHandler(req, res) {
    ...
    try {
        ...
        const uploadedFiles = Object.values(req.files).flat() as fileUpload.UploadedFile[];
...
export async function uploadLinkHandler(req, res) {
    const MAX_TITLE_LENGTH = 100;
    const postID = uuidv4();
